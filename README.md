...
A web app that monitors insider (SEC Form 4) and institutional (13F) buying — a free, self-hosted take on Autopilot-style trade tracking.
...
Everything runs on free tiers: SEC EDGAR (data), Supabase (Postgres + auth), Vercel (frontend), GitHub Actions (scheduled ingestion), Resend (email), Yahoo Finance (EOD prices).
...
- `ingester/` — Go module: EDGAR Form 4 + 13F ingestion, Yahoo Finance prices, alert dispatch
- `supabase/migrations/` — Postgres schema, RLS policies, cluster-buy view
